...
def main():
    ...
    # Step 2: Extract and Load Data in chunks
    train_df = load_and_process_data(os.path.join(output_data_path, 'train.csv'), chunk_size)
    test_df = load_and_process_data(os.path.join(output_data_path, 'test.csv'), chunk_size)
    ...
    all_data = all_data.sample(frac=.01, random_state=42).astype('float32')
    print("all_data size after reduction: {}".format(all_data.shape))
